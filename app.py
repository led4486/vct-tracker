...
with tab1:
    ...
    # ==========================================
    st.markdown("---")
    st.markdown("### ⚖️ VCT 공식 챔피언십 포인트 동률 결정 프로세스 안내")

    with st.container(border=True):
        st.markdown(
            """
            - **이미지 데이터 완벽 일치 반영**: 첨부해주신 이미지의 Stage 1, 2 그룹스테이지 결과 기반으로 합산 승수, 합산 맵 득실, 합산 라운드 득실이 코드 내에 정확히 반영되었습니다.
            - **노출 우선순위 정렬**: 지정된 **Stage 2 PO 1위팀과 2위팀**이 테이블의 가장 최상단에 우선 노출됩니다.
            - **진출 및 동률 하이라이트**: 진출이 확정된 팀은 **노란색**으로, 그 외 동률인 팀들은 **주황색/빨간색**으로 구분되어 하이라이트 됩니다.
            """
        )