...
with tab1:
    ...
    # ----------------------------------------------------
    # 사용자가 승자를 선택하여 3위나 4위가 완벽히 확정된 팀 색출
    ...
    # 해당 팀의 Stage 2 포인트와 Total Points 상승!
    if fixed_4th:
    ...
    # ----------------------------------------------------
    m_cond_text = [
        ("VARREL 승", "GE 승"), ("PRX 승", "ONG 승"), ("KRX 승(T1패)", "T1 승"),
        ("Nongshim 승", "어퍼세미 승자 승"), ("LR2(1) 상위팀 승", "LR2(1) 하위팀 승"), ("Gen.G 승", "LR1(2) 승자 승"),
        ("LR3 윗팀 승", "LR3 아랫팀 승"), ("Lower Final 윗팀 승", "Lower Final 아랫팀 승"), ("결승 직행팀 우승", "패자부활팀 우승")
    ]
    ...
    def get_dynamic_condition(tname, prob, universes):
        ...
        # AI 100% 힌트 서치 (누가 이기면 내가 100%가 되는가?)
        hint = ""
        for m_idx in range(9):
            if m_idx not in fixed_outcomes:
                t1, t2 = match_labels[m_idx]
                if t1.startswith("[") or t2.startswith("["): continue # 아직 매치업 이름이 안정해진 미래의 경기는 힌트 생략
    ...
    st.markdown("---")
    st.info("💡 **알고리즘 노트:** 사용자가 **'Lower R3 승자'**나 **'Lower Final 승자'** 등 특정 경기의 승자를 선택하여 패자가 완전히 확정되면, 표의 **[Stage 2]**와 **[Total Points]**에 **+4점, +5점이 즉시 합산되어 표시**됩니다!")
